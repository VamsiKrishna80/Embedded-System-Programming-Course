...
int main(void)
{
	...
	*pGPIOclken|=(1<<3);                                    //Enabling CLOCK to the GPIO port D
    ...
    /* Loop forever */
     while(1)
     	{
    	     if((GPIOD->IDR & 0x0002)==0x0002 || (GPIOD->IDR & 0x0080)==0x0080 || (GPIOD->IDR & 0x0010)==0x0010 || (GPIOD->IDR & 0x0040)==0x0040)
             {
                 delay();
                 count1=count1+1;
             }
             ...
             if((GPIOD->IDR & 0x0001)==0x0001 || (GPIOD->IDR & 0x0004)==0x0004 || (GPIOD->IDR & 0x0010)==0x0010 || (GPIOD->IDR & 0x0020)==0x0020)
                          {
                              delay();
                              count2=count2+1;
                          }
